...
///
/// `mouse on` also hands tmux the BUTTONS, which is not wanted and must NOT be fixed by
/// turning it off: in a pane running a bare shell tmux answers a drag with `copy-mode -M`
/// and a right-click with its own Split/Kill `display-menu`. The frontend takes those back
/// by refusing to enter mouse reporting in a shell pane and re-encoding the wheel by hand
/// -- see `src/terminalMouse.ts`. Flipping this to `off` would break that wheel instead of
/// helping, because tmux only reads mouse input while this is on.
